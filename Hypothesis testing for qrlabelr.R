...
gtext_mac <- paste(paste0('t-statistic = ', round(ttest_mac$statistic, 2), ';'),
                   paste0('df = ', round(ttest_mac$parameter, 1), ';'),
...
gtext_win <- paste(paste0('t-statistic = ', round(ttest_win$statistic, 2), ';'), 
                   paste0('df = ', round(ttest_win$parameter, 1), ';'),
...
pp <- pp +  annotate("table_npc", npcx = "center", npcy = "center",
               label = list(summ_mac), size = 5)
...
ppp <- ppp + theme(text = element_text(size = 18, color = 'black'))
ppp
################################################################################
...
ggsave(filename = paste0("violin_plots",  ".pdf"), 
       ...
       width = 13.5, height = 5, onefile = TRUE)
